...
def checksol(data): 
    ...
    predPE =  0.01*uux2 -0.0002*uxu2x -0.0001*uu3x + 0.005*u2u2x +5.0e-07*u4x-0.00025*uu2x
...
data = pd.read_csv("./../data_gen/data/mod_burgW_data.csv",  encoding='utf-8') #%%Check the solution 
data = data.iloc[::10]
#checksol(data)
...
lam1 = data.columns.get_loc("uux2")  + 1   #  0.01     uux2 34
lam2 = data.columns.get_loc("uxu2x") + 1   # -0.0002   uxu2x 14
lam3 = data.columns.get_loc("uu3x")  + 1   # -0.0001   uu3x 10
lam4 = data.columns.get_loc("u2u2x") + 1   #  0.005    u2u2x 30
lam5 = data.columns.get_loc("u4x")   + 1   #  5.0e-07  u4x  8
lam6 = data.columns.get_loc("uu2x")  + 1   # -0.00025  uu2x 9

#%%
key= '''
#Using STRidge to predict the PDE uer =  0.01*uux2 -0.0002*uxu2x -0.0001*uu3x + 0.005*u2u2x  + 5.0e-07*u4x - 0.00025*uu2x
'''
n_alphas = 200
alphas = np.logspace(-7.6, -7, n_alphas)
dtol = 1e-4
...
#%%Psot processing
# 0.01*uux2 -0.0002*uxu2x -0.0001*uu3x +0.005*u2u2x +5.00e-07*u4x 
ax = plt.gca()
for i in range(coef.shape[1]):
    if i == lam1:
        ax.plot(alphas, coef[:,i],'b--',lw=2, label = '$uu_x^2$')    
    elif i== lam2:
        ax.plot(alphas, coef[:,i],'r--',lw=2, label = '$u_xu_{2x}$') 
    elif i== lam3:
        ax.plot(alphas, coef[:,i],'g--',lw=2, label = '$ uu_{3x}$')   
    elif i== lam4:
        ax.plot(alphas, coef[:,i],'c--',lw=2, label = '$ u^2 u_{2x}$') 
    elif i== lam5:
        ax.plot(alphas, coef[:,i],'y--',lw=2, label = '$ u_{4x}$')    
    elif i== lam6:
        ax.plot(alphas, coef[:,i],'k--',lw=2, label = '$ uu_{2x}$')   
...
plt.legend(loc=2)
plt.ylim(-3e-3,1.25e-2)
plt.show()


#%%
#np.savez('coeff_data/mod_burW_str0.npz', coef=cf, dtol= otol, lam=alphas, test_mse=test_mse)
