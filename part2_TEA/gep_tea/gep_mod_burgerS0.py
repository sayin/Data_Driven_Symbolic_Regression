...
data = pd.read_csv("./../data_gen/data/mod_burgS_data.csv",  encoding='utf-8')
...
h         = 8         # head length t = h(n-1) + 1 5, 8
n_genes   = 4         # number of genes in a chromosome 4
r         = 5         # length of the RNC array  5
enable_ls = True      # whether to apply the linear scaling technique

# size of population and number of generations10
n_pop   = 50
n_gen   = 500
champs  = 3 
...
pset.add_ephemeral_terminal(name='enc', gen=lambda: random.uniform(-2.5e-5,2.5e-5)) 
pset.add_ephemeral_terminal(name='enc', gen=lambda: random.uniform(-5e-7,2.5e-7)) 
pset.add_ephemeral_terminal(name='enc', gen=lambda: 1.25e-5) 
...
uer = 2.5e-05*uux2 -5.0e-07*uxu2x -2.5e-07*uu3x + 1.25e-05*u2u2x + 1.25e-09*u4x - 1.25e-05*uu2x
...
def CalculateBestModelOutput(u,ux,u2x,u3x,u4x, model):
    ...
    return eval(model) 
#
predPE = CalculateBestModelOutput(holdout.u,holdout.ux, holdout.u2x,holdout.u3x,holdout.u4x,str(symplified_best))
...
#%%
#
path = 'results/burgs/gep_mod_burgS.txt'
...
file.writelines("%s \n%s %s \n%s %s\n%s %s\n%s %s\n%s %s \n%s %s %s \n%s %s\n%s %s\n%s %s\n%s %s\n%s \n" % ('settings', 
      ...
      'best indices =', best_ind.a, best_ind.b,
        'best model =',str(symplified_best),
         'target model =',' 2.5e-05*uux2 -5.0e-07*uxu2x -2.5e-07*uu3x + 1.25e-05*u2u2x + 1.25e-09*u4x - 1.25e-05*uu2x',
        'Test_MSE = ', test_mse,
        'Test_R2 = ', test_r2,
          log))
file.close()
...
#skp = 10
#file = 'results/burgs/burgS_pred.pdf'
...
#gep.export_expression_tree(best_ind, rename_labels, 'results/burgs/burgs_tree.pdf')
