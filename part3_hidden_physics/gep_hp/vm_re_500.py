...
path = './../00_data_gen/derivatives/vm_re_500/'
...
##Testing data ###
#test_mse1 =  mean_squared_error(star,stru)
#test_r21  =  r2_score(star,stru)
#
#et =np.exp(-(2*np.pi)**2*t/500)
#stpr = 0.01*np.sin(x)*np.cos(y)*et
#test_mse2 =  mean_squared_error(star,stpr)
#test_r22  =  r2_score(star,stpr)
...
def evaluate(individual):
    ...
    Yp = np.array(list(map(func,x, y, t)),dtype='float64') 
...
pset.add_ephemeral_terminal(name='enc', gen=lambda: random.uniform(-0.01, 0.1)) 
...
key= '''
Using GEP to predict the PDE t11 = 0.01*sin(x)*cos(y)*e^(-nu*t)
...
###
# In[75]:
#predPE = -1.47982886248023e-6 + 0.00998104698959112*np.exp(-0.0788118792085216*t)*np.sin(x)*np.cos(y)
#from sklearn.metrics import mean_squared_error, r2_score
...
#        'target model =','0.01*sin(x)*cos(y)*e^(-0.0789568*t)',
#        'Test_MSE = ', test_mse,
#        'Test_R2 = ',  test_r2,
#          log))
#
