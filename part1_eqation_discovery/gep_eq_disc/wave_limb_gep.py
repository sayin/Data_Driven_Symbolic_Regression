...
## Symbolic Regression on Wave data
...
waveData = pd.read_csv("./../../../data_gen/clean/data/wave_data.csv",  encoding='utf-8')       

# In[46]:

msk = np.random.rand(len(waveData)) < 0.8
train = waveData[msk]
holdout = waveData[~msk]
...
key= '''
Using GEP to predict the PDE ut = -1.0 ux
...
#print("R2 score : %.2f" % r2_score(holdout.ut, predPE))


#%%

#path = 'results/wave_log.txt'
#
...
#gep.export_expression_tree(best_ind, rename_labels, 'results/wave_tree_02.pdf')
