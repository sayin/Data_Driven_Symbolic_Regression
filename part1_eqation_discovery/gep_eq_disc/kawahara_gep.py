...
kawData = pd.read_csv("./../../../data_gen/clean/data/kaw_data.csv",  encoding='utf-8')
# In[46]:

msk = np.random.rand(len(kawData)) < 0.8
train = kawData[msk]
holdout = kawData[~msk]
...
def evaluate(individual):
    ...
    Yp = np.array(list(map(func, u,u2x,u3x,u4x,u5x,uux)))   
...
# In[51]:
pset = gep.PrimitiveSet('Main', input_names=['u','u2x','u3x','u4x','u5x','uux'])
...
key= '''
Using GEP to predict the PDE ut = -uux - u3x + u5x
...
# In[70]:
#def CalculateBestModelOutput(u,uux,u2x,u3x,u4x,u5x, model):
#    return eval(model) 
#
#predPE = CalculateBestModelOutput(holdout.u,holdout.uux, holdout.u2x,holdout.u3x,holdout.u4x,holdout.u5x,str(symplified_best))
##
# In[75]:
#
...
#gep.export_expression_tree(best_ind, rename_labels, 'results/kaw_tree.pdf')
...
#rename_labels = {'add': '+', 'sub': '-', 'mul': '*', 'protected_div': '/', 'sin': 'sin', 'cos': 'cos', 'tan': 'tan'}  
#gep.export_expression_tree(best_ind, rename_labels, 'results/kaw_tree_0.pdf')
