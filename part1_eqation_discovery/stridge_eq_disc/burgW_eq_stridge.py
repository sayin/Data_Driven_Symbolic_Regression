...
data = pd.read_csv("./../data_gen/clean/data/burgerw_data.csv",  encoding='utf-8') #%%Check the solution 
data = data.iloc[::10]
checksol(data)
...
for i in range(coef.shape[1]):
    if i ==lam1:
        ...
        ax.plot(alphas, coef[:,i]/0.01,'r--',lw=2)
#        pass  
    else:
...
np.savez('coeff_data/burgW_str0.npz', coef=cf, dtol= otol, lam=alphas, test_mse=test_mse)