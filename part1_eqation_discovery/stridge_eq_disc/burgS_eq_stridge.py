...
data = pd.read_csv("./../data_gen/clean/data/burgers_data.csv",  encoding='utf-8') #%%Check the solution 
...
theta = np.column_stack((c,theta))  ## Stack const =1--columnn  to theta


lam1 = data.columns.get_loc("uux")  + 1   #uux
lam2 = data.columns.get_loc("u2x")  + 1   #u2x

#%%
key= '''
#Using STRidge to predict the PDE ut = -u*ux + 0.01u2x 
'''

n_alphas = 50
alphas = np.logspace(-5, -0.1, n_alphas)
dtol = 0.1
...
for i in range(coef.shape[1]):
    if i ==lam1:
        ...
        ax.plot(alphas, coef[:,i]/0.01,'r--',lw=2, label = '$u_{2x}$')
        pass  
    else:
...
np.savez('coeff_data/burgS_str0.npz', coef=cf, dtol= otol, lam=alphas, test_mse=test_mse)