...
data = pd.read_csv("./../data_gen/clean/data/kdv_data.csv",  encoding='utf-8') #%%Check the solution 
data = data.iloc[::20]
checksol(data)
...
lam2 = data.columns.get_loc("u3x")  + 1   #u3x
#%%
key= '''
#Using STRidge to predict the PDE ut = -6u*ux - u3x 
'''
...
for i in range(coef.shape[1]):
    if i == lam1:
        ax.plot(alphas, coef[:,i]/6,'b--',lw=2)
    elif i== lam2:
...
np.savez('coeff_data/kdv_str0.npz', coef=cf, dtol= otol, lam=alphas, test_mse=test_mse)