...
data = pd.read_csv("./../data_gen/clean/data/heat_data.csv",  encoding='utf-8') #%%Check the solution 
data = data.iloc[::10]
#checksol(data)
#
...
lam1 = data.columns.get_loc("u2x")  + 1   #u2x

#%%
key= '''
#Using STRidge to predict the PDE ut =  u2x
'''
...
for i in range(coef.shape[1]):
    if i == lam1:
        ax.plot(alphas, coef[:,i],'b--',lw=2, label = '$u_{2x}$')
    else:
...
np.savez('coeff_data/heat_str0.npz', coef=cf, dtol= otol, lam=alphas, test_mse=test_mse)