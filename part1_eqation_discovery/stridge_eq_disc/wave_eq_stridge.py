...
data = pd.read_csv("./../data_gen/clean/data/wave_data.csv",  encoding='utf-8') #%%Check the solution 
data = data.iloc[::10]
#checksol(data)
#
...
lam1 = data.columns.get_loc("ux")  + 1   #ux


#%%
key= '''
#Using STRidge to predict the PDE ut = -1.0 ux
'''

n_alphas = 100
alphas = np.logspace(-20, -1, n_alphas)
dtol = 1e-09
...
for i in range(coef.shape[1]):
    if i == lam1:
        ax.plot(alphas, coef[:,i],'b--',lw=2, label = '$u_x$')
    else:
        ax.plot(alphas, coef[:,i], lw=2)    
#        pass  

pdf = 'results/wave_str0_'+str(dtol).replace('.','')+'.pdf'        
eps= 'results/wave_str0_' +str(dtol).replace('.','')+'.eps'        
ax.set_xscale('log')
...
plt.show()
#%%
np.savez('coeff_data/wave_str0.npz', coef=cf, dtol= otol, lam=alphas, test_mse=test_mse)