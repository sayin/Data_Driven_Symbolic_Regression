...
data = pd.read_csv("./../data_gen/clean/data/newwell_whitehead_segel_data.csv",  encoding='utf-8') #%%Check the solution 
data = data.iloc[::10]
checksol(data)
...
lam1 = data.columns.get_loc("u2x")   #u2x
lam2 = data.columns.get_loc("u") +1    #u
lam3 = data.columns.get_loc("u2")    #u2

#%%
key= '''
#Using STRidge to predict the PDE ut = u2x + u - u**2
'''
...
np.savez('coeff_data/nws_str0.npz', coef=cf, dtol= otol, lam=alphas, test_mse=test_mse)